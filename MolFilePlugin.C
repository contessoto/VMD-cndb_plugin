 ...
 *      $Revision: 1.198 $      $Date: 2020/10/22 03:43:23 $
 *
 ***************************************************************************
 * DESCRIPTION:
...
int MolFilePlugin::write_structure(Molecule *m, const int *on) {
  ...
  for (i=0, j=0; i<m->nAtoms; i++) {
    ...
    if (mangleatomnames) {
      ...
      strncpy(atm.name, m->atomNames.name(atom->nameindex), sizeof(atm.name)-1);
    }
  ...
  if (can_write_bonds() && 
    ...
    for (i=0; i<m->nAtoms; i++) {
      const MolAtom *atom = m->atom(i);
      ptrdiff_t bfmap = atomindexmap[i] + 1; // 1-based mapped atom index
...
int MolFilePlugin::read_volumetric(Molecule *m, int nsets, const int *setids) {
  ...
  for (int i=0; i< n; i++) {
    ...
    ptrdiff_t size = ptrdiff_t(v->xsize) * ptrdiff_t(v->ysize) * ptrdiff_t(v->zsize);
