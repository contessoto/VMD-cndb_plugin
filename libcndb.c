...
int get_box_information(struct cndb_file* file, int group_number, int time_i, cndb_box* out_box);
int get_box_vectors(struct cndb_file* file,  int group_i, int time_i, float* vector_a, float* vector_b, float* vector_c);
...
int discover_all_groups(struct cndb_file* file){
...
int cndb_close(struct cndb_file* file){
	if(file!=NULL){
		...
		free(file);	//free cndb_file struct
...
void sort_data_according_to_id_dataset(struct cndb_file* file, int group_number, float* to_be_sorted_data){
	#if defined DEBUG
	cndb_show_hdf5_error_messages();
	#endif
	int i= group_number;
	/////////////////
...
int get_box_vectors(struct cndb_file* file,  int group_i, int time_i, float* vector_a, float* vector_b, float* vector_c){
	...
	if(box_timedependent_dataset_id>=0){
		...
		int dims_box;
		int is_cubic=cndb_get_length_of_one_dimensional_dataset(file, full_path_box_dataset_timeindependent, &dims_box);
		float* data_box;
		H5T_class_t box_class_out;
		cndb_read_timeindependent_dataset_automatically(file, full_path_box_dataset_timeindependent,(void**) &data_box, &box_class_out);
		if(is_cubic==0){
...
int cndb_read_timeindependent_dataset_int(struct cndb_file* file, char* dataset_name, int ** _data_out){
	...
	int status_read=cndb_read_timeindependent_dataset_automatically(file, dataset_name, (void**)_data_out, &type_class_out);
...
int cndb_get_file_id(struct cndb_file *file, hid_t *file_id){
...
/* write operations */
//creates a cndb_file iff it does not exist yet
int cndb_create_file(struct cndb_file **_file, const char* filename){
	struct cndb_file *file = (struct cndb_file*) malloc(sizeof(struct cndb_file));
	initialize_cndb_struct(file);
	...
	cndb_set_author(file, NULL, NULL); //sets author name by default to the user account name, can be overwritten by another call of cndb_set_author(file,"username");
	cndb_set_creator(file, NULL, NULL);
	*(_file)=file;
	if(file_id<0){
...
int cndb_append_dataset(struct cndb_file *file, char* absolute_name_of_dataset, hid_t datatype, void* data_in, int rank_in, hsize_t* dims_in){
	int status;
	...
	if(dataset_id>=0){
		...
		status=cndb_write_dataset(file, absolute_name_of_dataset, datatype, data_in, rank_in, dims_in);
	}
	return status;
}
...
int cndb_delete_dataset(struct cndb_file* file, char* absolute_name_of_dataset){ //absolute_name_of_dataset="absolute_path/name_of_dataset"
...
int cndb_set_author(struct cndb_file* file, char* name, char* email_address){
	...
	H5Gcreate(file->file_id, "/cndb/author", link_crt_plist, H5P_DEFAULT, H5P_DEFAULT);

	herr_t status_name =-1;
	if(name == NULL){
		char* username=getlogin();
		status_name=H5LTset_attribute_string(file->file_id, "/cndb/author", "name", username);
	}else{
		status_name=H5LTset_attribute_string(file->file_id, "/cndb/author", "name", name);
	}

	int status_email =-1;
	if(email_address==NULL){
		status_email=H5LTset_attribute_string(file->file_id, "/cndb/author", "email", "email was not provided");
	}else{
		status_email=H5LTset_attribute_string(file->file_id, "/cndb/author", "email", email_address);
	}
...
int cndb_set_creator(struct cndb_file* file, char* name, char* version){
	...
	H5Gcreate(file->file_id, "/cndb/creator", link_crt_plist, H5P_DEFAULT, H5P_DEFAULT);

	herr_t status_name =-1;
	if(name == NULL){
		status_name=H5LTset_attribute_string(file->file_id, "/cndb/creator", "name", "libcndb");
	}else{
		status_name=H5LTset_attribute_string(file->file_id, "/cndb/creator", "name", name);
	}

	int status_version =-1;
	if(version==NULL){
		status_version=H5LTset_attribute_string(file->file_id, "/cndb/creator", "version", "version not provided");
	}else{
		status_version=H5LTset_attribute_string(file->file_id, "/cndb/creator", "version", version);
	}
...
//int cndb_set_version(struct cndb_file* file, char* name){

//}

/*
int cndb_delete_attribute(){

}

int cndb_rename_dataset(){
